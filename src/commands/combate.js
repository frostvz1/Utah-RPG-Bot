...
const { getUser, updateUser, adicionarXP } = require("../systems/users");
...
async function combate(
  ...
  if (
    ...
    if (
      ...
      return sendGifMessage(
        ...
        `Use ${prefix}habilidade remover <número>.\n` +
        "Exemplo: " +
        `${prefix}habilidade remover 1`
      );
    }
    ...
    return sendGifMessage(
      ...
      "       HABILIDADE REMOVIDA\n" +
      "╚════════════════════════════╝\n\n" +
      `Habilidade: ${habilidadeRemovida}\n` +
      `Espaço removido: ${numero}\n\n` +
      `Habilidades restantes: ${novaLista.length}/5`
    );
  }
  ...
  if (enemy.hp <= 0) {
    ...
    const texto =
      "╔════════════════════════════╗\n" +
      ("          " + (require("../systems/messages").getMessage("victory") || "VITÓRIA") + "\n") +
      "╚════════════════════════════╝\n\n" +
      `Inimigo: ${enemy.name}\n` +
      `Dano causado: ${damage}\n` +
      (
      ...
      `XP recebido: +${xpGain}\n` +
      (
        resultadoXP.niveisGanhos > 0
          ? `NÍVEL AUMENTADO: ${resultadoXP.nivelAnterior} → ${resultadoXP.nivelAtual}\n`
          : ""
      ) +
